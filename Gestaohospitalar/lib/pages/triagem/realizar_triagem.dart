...
class RealizarTriagem extends StatefulWidget {
  ...
  const RealizarTriagem(
      {super.key, required this.pacienteService, required this.triagemService});
  ...
  Future<void> _enviarParaFilaMedica() async {
    if (_formKey.currentState!.validate() && _pacienteSelecionado != null) {
      try {
        ...
        if (mounted) {
          Navigator.pop(context); // Fecha a triagem e volta pra Dashboard
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Triagem finalizada. Paciente na fila!'), backgroundColor: Colors.teal)
          );
        }
      } catch (e) {
        if (mounted) {
        ...
        child: Column(
          children: [
            Row(
              ...
              children: [
                const Row(children: [
                  Icon(Icons.medical_information, color: Colors.teal, size: 28),
                  SizedBox(width: 8),
                ...
                child: Column(
                  ...
                  children: [
                    DropdownButtonFormField<Paciente>(
                      ...
                      items: widget.pacienteService.pacientes
                          .map((p) => DropdownMenuItem(
                              value: p, child: Text("${p.nome}")))
                          .toList(),
                      onChanged: (p) {
                        setState(() {
                    ...
                    TextFormField(
                        ...
                        decoration: const InputDecoration(
                            labelText: "Alergias Detectadas",
                            border: OutlineInputBorder())),
                    const SizedBox(height: 16),
                    const Text("Protocolo de Manchester",
                        style: TextStyle(
                            fontWeight: FontWeight.bold, color: Colors.grey)),
                    const SizedBox(height: 12),
                    Wrap(
                      ...
                      children: [
                        ...
                        return ChoiceChip(
                          ...
                          onSelected: (bool selected) {
                            if (selected) {
                              setState(() => _riscoSelecionado = risco);
                            }
                          },
                        );
                      }).toList(),
              ...
              children: [
                ...
                ElevatedButton.icon(
                  style: ElevatedButton.styleFrom(
                      backgroundColor: _obterCorRisco(_riscoSelecionado),
